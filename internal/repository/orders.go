...
func (d *DBStorage) CheckOrderOwnership(orderNumber string, currentUserID int64) (models.OrderCheckResult, error) {
	var dbUserID int64

	query := `SELECT user_id FROM orders WHERE number = $1 LIMIT 1;`

	err := d.db.QueryRowContext(d.ctx, query, orderNumber).Scan(&dbUserID)
	if err != nil {
...
func (d *DBStorage) CreateOrder(userId int64, number string, result models.OrderResult) (int, error) {
	...
	query := `
		...
		RETURNING id;
	`
	err := d.db.QueryRowContext(d.ctx, query, userId, number, result.Status, result.Accrual, now).Scan(&lastInsertID)
...
func (d *DBStorage) GetOrdersByUserID(userID int64) ([]models.OrderListItem, error) {
	query := `
